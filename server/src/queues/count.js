...
consumer.task = async function (job, done) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        case 'mined':
            ...
            break
        case 'inTx':
            await db.Account.updateOne({ hash: hash }, { $inc : { inTxCount: 1, totalTxCount: 1 } }, { upsert: true })
            break
        case 'outTx':
            await db.Account.updateOne({ hash: hash }, { $inc : { outTxCount: 1, totalTxCount: 1 } }, { upsert: true })
            break
        case 'internalTx':
